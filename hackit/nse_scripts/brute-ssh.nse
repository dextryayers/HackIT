...
local function ssh_make_packet(pkt_type, payload)
  ...
  local pkt = string.char(
    bit.rshift(total_len, 24) % 256,
    bit.rshift(total_len, 16) % 256,
    bit.rshift(total_len, 8) % 256,
    total_len % 256
  )
  pkt = pkt .. string.char(pad_len) .. string.char(pkt_type) .. payload
  for i = 1, pad_len do
    pkt = pkt .. string.char(math.random(1, 255))
  end
  return pkt
end
...
action = function(host, port)
  ...
  local kex_cookie = ""
  for i = 1, 16 do
    kex_cookie = kex_cookie .. string.char(math.random(0, 255))
  end
    ...
    for _, p in ipairs(passes) do
      ...
      local ok, result = pcall(function()
        ...
        if not kex_type then
          local auth_payload = string.char(0, 0, 0, #u) .. u
            .. string.char(0, 0, 0, 14) .. "ssh-connection"
            .. string.char(0, 0, 0, 8) .. "password"
            .. string.char(0)
            .. string.char(0, 0, 0, #p) .. p
