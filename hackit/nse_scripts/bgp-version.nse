...
local function bgp_open_message(asn, hold_time, bgp_id)
  local marker = string.rep(string.char(0xff), 16)
  local length = 29
  local type = 1
  local version = 4
  ...
  local id_bytes = {}
  for octet in string.gmatch(bgp_id or "1.2.3.4", "%d+") do
    table.insert(id_bytes, string.char(tonumber(octet)))
  end
  local bgp_id_str = table.concat(id_bytes)
  local opt_param_len = string.char(0)
  local header = marker .. bin.pack(">i", length):sub(3, 4) .. string.char(type)
  local body = string.char(version) .. my_as .. hold .. bgp_id_str .. opt_param_len
  return header .. body
end
...
local function parse_bgp_open(response)
  ...
  if #response >= 19 then
    ...
    info.message_name = msg_types[type_byte] or string.format("Unknown (%d)", type_byte)
    ...
    if type_byte == 2 and #response >= 29 then
      ...
      if response:byte(25) and response:byte(26) and response:byte(27) and response:byte(28) then
        info.router_id = string.format("%d.%d.%d.%d", response:byte(25), response:byte(26), response:byte(27), response:byte(28))
      end
...
action = function(host, port)
  ...
  if not ok then
    return stdnse.format_output(false, "Could not connect to BGP port: " .. tostring(err))
  end
  ...
  if not ok2 then
    socket:close()
    return stdnse.format_output(false, "Failed to send BGP OPEN: " .. tostring(serr))
  end
  ...
  if not ok3 or not response or #response < 19 then
    return stdnse.format_output(false, "No BGP OPEN response received")
  end
