  ...
  local ok, result = pcall(function()
    ...
    for _, vec in ipairs(vectors) do
      ...
      if req and req.status then
        ...
        if vec.method == "PROPFIND" and (req.status == 207 or (req.status < 300 and body:match("DAV:"))) then
          table.insert(findings, {vector = ("%s %s"):format(vec.method, vec.path), status = req.status, detail = "WebDAV PROPFIND succeeded"})
        elseif vec.method == "OPTIONS" then
          local allow = (req.headers and req.headers["allow"]) or ""
          local all = type(allow) == "table" and table.concat(allow, ", ") or allow
          if all:match("PROPFIND") or all:match("MKCOL") or all:match("MOVE") then
            table.insert(findings, {vector = "OPTIONS /", status = req.status, detail = ("WebDAV methods allowed: %s"):format(all)})
          end
        elseif vec.method == "PUT" and req.status == 201 then
          table.insert(findings, {vector = "PUT /hackit_test.txt", status = req.status, detail = "Write access confirmed - file upload succeeded"})
          http.generic_request(host, port, "DELETE", "/hackit_test.txt", {}, "")
        elseif vec.method == "MKCOL" and req.status == 201 then
          table.insert(findings, {vector = "MKCOL /hackit_test_dir/", status = req.status, detail = "Directory creation allowed"})
    ...
    if #findings > 0 then
      ...
      if server_banner then
        ...
        if ver then
          local parts = {}
          for v in ver:gmatch("%d+") do table.insert(parts, tonumber(v)) end
          if #parts >= 2 then
