...
action = function(host, port)
    ...
    for _, pattern in ipairs(test_patterns) do
        ...
        if resp and resp.status then
            if resp.status == 200 or resp.status == 301 or resp.status == 302 then
                ...
                insert(results, "Shortname response at " .. pattern .. " (status " .. resp.status .. ")")
            end
        end
    end
    ...
    if aspnet_resp and aspnet_resp.header then
        ...
        if x_aspnet then
            insert(results, "ASP.NET version: " .. x_aspnet)
        end
    end
    if #results == 0 then
        return format_output(false, "No IIS shortname vulnerability detected")
    end
