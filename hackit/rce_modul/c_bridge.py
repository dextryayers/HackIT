...
class CEngine:
    def __init__(self):
        ...
        self.c_dir = os.path.join(self.base_dir, 'c')
        exe = '.exe' if platform.system() == 'Windows' else ''
        self.binary_name = f'rce_engine{exe}'
        self.binary_path = os.path.join(self.c_dir, 'bin', self.binary_name)
        self.source_path = os.path.join(self.c_dir, 'main.c')
    ...
    def ensure_compiled(self) -> bool:
        if os.path.exists(self.binary_path):
            ...
            if src_mtime <= bin_mtime:
                return True
        try:
            os.makedirs(os.path.join(self.c_dir, 'bin'), exist_ok=True)
            result = subprocess.run(
                ['gcc', '-Wall', '-O2', '-std=c11', '-o', self.binary_path, 'main.c'],
                cwd=self.c_dir, check=True, capture_output=True, text=True
            )
            return True
        except subprocess.CalledProcessError as e:
            print(f"[C Build Error] {e.stderr}", file=sys.stderr)
            return False
    ...
    def run(self, url: str, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result.stderr:
                print(f"[C Engine] {result.stderr}", file=sys.stderr)
            if kwargs.get('json', True):
                try:
        ...
        for line in output.strip().split('\n'):
            ...
            if parts[0] == 'VULNERABLE' and len(parts) >= 6:
                results.append({
                    ...
                    'output': parts[5], 'engine': 'c'
                })
            elif parts[0] == 'SAFE':
                results.append({'vulnerable': False, 'url': parts[1], 'engine': 'c'})
        return results if results else [{'vulnerable': False, 'engine': 'c', 'note': 'parse_fallback'}]
