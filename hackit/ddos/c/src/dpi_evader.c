                            ...
                            const dpi_config_t *cfg)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (is_last) {
            ...
            frag_size = avg + (rand() % (max_sz - min_sz + 1));
            if (frag_size < min_sz)
                frag_size = min_sz;
            if (frag_size > max_sz)
                frag_size = max_sz;
                           ...
                           const dpi_config_t *cfg)
{
    ...
    for (int i = 0; i < frag_count; i++) {
        ...
        ret = (int)sendto(sock_raw, pkt, (size_t)total_pkt, 0,
                          (struct sockaddr *)&sin, sizeof(sin));
        if (ret > 0)
            total_sent++;

        if (cfg->interleave_delay_us > 0)
            usleep((unsigned int)cfg->interleave_delay_us);
    }
