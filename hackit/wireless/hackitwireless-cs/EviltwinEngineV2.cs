...
{
    public class EviltwinEngineV2
    {
        private readonly string _iface;
        private readonly string[] _ssids;
        private readonly byte[][] _bssids;
        ...
        public EviltwinEngineV2(string iface, string[] ssids, string[] bssids, int channel)
        {
            _iface = iface;
            _ssids = ssids;
            _bssids = new byte[bssids.Length][];
            for (int i = 0; i < bssids.Length; i++)
                _bssids[i] = ParseMac(bssids[i]);
            _channel = channel;
        }
        ...
        private void BeaconLoop()
        {
            ...
            try
            {
                while (_running)
                {
                    ...
                    {
                        int cur = idx % _ssids.Length;
                        idx++;
                        byte[] f = BuildBeaconFrame(_bssids[cur % _bssids.Length], _ssids[cur], _channel, seq++);
