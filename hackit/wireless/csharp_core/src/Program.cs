...
namespace HackITWireless
{
    class Program
    {
        ...
        {
            if (args.Length < 4) { Console.Error.WriteLine("Usage: deauth <bssid> <station> <count> <interface>"); return 1; }
            var engine = new AttackEngine();
            string bssid = args[1], station = args[2];
            int count = int.Parse(args[3]);
            string iface = args.Length > 4 ? args[4] : "wlan0";
        ...
        {
            if (args.Length < 3) { Console.Error.WriteLine("Usage: beacon <ssid> <count> <interface>"); return 1; }
            var engine = new AttackEngine();
            string ssid = args[1];
            int count = int.Parse(args[2]);
            string iface = args.Length > 3 ? args[3] : "wlan0";
        ...
        {
            if (args.Length < 3) { Console.Error.WriteLine("Usage: handshake <bssid> <timeout> <interface> [output]"); return 1; }
            var engine = new AttackEngine();
            string bssid = args[1];
        ...
        {
            if (args.Length < 2) { Console.Error.WriteLine("Usage: wps <bssid> <interface> [method]"); return 1; }
            var engine = new AttackEngine();
            string bssid = args[1];
            string iface = args.Length > 2 ? args[2] : "wlan0";
        ...
        {
            string iface = args.Length > 1 ? args[1] : "wlan0";
