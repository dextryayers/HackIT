...
async def query_geo(ip: str, name: str, url_tmpl: str, client) -> dict:
    url = url_tmpl.format(ip)
    resp = await safe_fetch(client, url, headers={"User-Agent": UA, "Accept": "application/json"})
...
async def check_proxy_vpn(ip: str, client) -> list:
    results = []
    resp = await safe_fetch(client, f"https://ipqualityscore.com/api/json/ip/{ip}")
    if resp and resp.status_code == 200:
        data = resp.json()
        ...
        for k in keys:
            ...
            if val:
                findings.append(make_finding(
                    ...
                    ftype=f"Geo Recon: {field.title()} ({source})",
                    source=source,
...
async def crawl(target: str, client) -> list:
    ...
    try:
        ...
        if not ip:
            findings.append(make_finding(
                ...
                ftype="Geo Recon: Invalid Target",
                source="GeoRecon",
                confidence="Low",
                color="slate",
                threat_level="Informational",
    ...
    for pv in proxy_vpn:
        findings.append(make_finding(
            ...
            ftype=f"Geo Recon: {pv.upper()} Detection",
            source="GeoRecon",
            confidence="Medium",
            color="orange",
