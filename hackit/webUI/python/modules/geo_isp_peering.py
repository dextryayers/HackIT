...
async def _get_asn_info(ip: str, client) -> list:
    findings = []
    ...
    if data:
        ...
        if asn_num:
            pd_data = await safe_fetch_json(client, PEERINGDB_URL.format(asn=asn_num),
...
async def crawl(target: str, client) -> list:
    ...
    if ip is None:
        findings.append(make_finding(entity=f"DNS resolution failed: {target}", ftype="DNS Error", source="ISPPeeringScanner", confidence="Low", color="red", category="Geo / Network OSINT", raw_data=str(is_ip_flag)[:200], tags=["error"]))
        return findings

    if not is_ip_flag:
        findings.append(make_finding(entity=f"{target} -> {ip}", ftype="DNS Resolution", source="ISPPeeringScanner", confidence="High", color="slate", category="Geo / Network OSINT", threat_level="Informational", status="Resolved", resolution=ip, tags=["dns", "resolution"]))
    ...
    findings.append(make_finding(entity=f"ISP tier classifications: {tier_count}", ftype="ISP Tier Count", source="ISPPeeringScanner", confidence="Medium", color="purple", category="Geo / Network OSINT", tags=["isp", "peering", "summary"]))
    findings.append(make_finding(entity=f"IXP references: {ixp_count}", ftype="IXP Count", source="ISPPeeringScanner", confidence="Medium", color="purple", category="Geo / Network OSINT", tags=["isp", "peering", "summary"]))
    findings.append(make_finding(entity=f"Target: {target}", ftype="ISP Peering Target", source="ISPPeeringScanner", confidence="High", color="slate", category="Geo / Network OSINT", tags=["isp", "peering", "target"]))
    findings.append(make_finding(entity=f"Resolved IP: {ip}", ftype="ISP Peering IP", source="ISPPeeringScanner", confidence="High", color="slate", category="Geo / Network OSINT", tags=["isp", "peering", "ip"]))
    findings.append(make_finding(entity=f"Total ISP peering findings: {len(findings)}", ftype="ISP Peering Summary", source="ISPPeeringScanner", confidence="Medium", color="purple", category="Geo / Network OSINT", tags=["isp", "peering", "summary"]))
