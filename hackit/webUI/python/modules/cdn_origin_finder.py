...
CDN_HEADERS = {
    ...
    "cloudfront": ["x-amz-cf-id", "x-amz-cf-pop", "x-cache", "via"],
    "akamai": ["x-akamai-", "akamai-", "x-akamai-transformed"],
    "fastly": ["x-served-by", "x-cache", "x-cache-hits", "x-timer", "fastly-"],
    "azure_cdn": ["x-azure-", "azurecdn"],
    "gcp_cdn": ["x-cloud-trace-context", "via"],
...
CNAME_CDN_PATTERNS = {
    ...
    "akamai": [".akamai", "akamaiedge", "akamaihd", ".akamaized.net"],
    "fastly": [".fastly.net", ".fastlylb.net"],
    "azure_cdn": [".azureedge.net", ".azurefd.net", ".trafficmanager.net"],
    ...
    "gcore": [".gdagent.com", ".gcorelabs.com"],
    "quantil": [".quantil.com"],
...
ALT_PORTS = [80, 443, 8080, 8443, 4443, 2053, 2083, 2087, 2096, 8880, 8888, 9000, 9001, 7443]
...
def detect_cdn_from_headers(headers: Dict) -> List[str]:
    ...
    for cdn_name, patterns in CDN_HEADERS.items():
        for pattern in patterns:
            if any(hk.startswith(pattern.lower()) for hk in headers_lower):
...
async def shodan_favicon_search(client: httpx.AsyncClient, domain: str) -> List[str]:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            search_url = f"https://www.shodan.io/search?query=http.favicon.hash:{fav_hash}"
            headers = {"User-Agent": UA}
            search_resp = await client.get(search_url, headers=headers, timeout=20.0)
...
async def crawl(target: str, client: httpx.AsyncClient):
    ...
    if detected_cdns and len(all_origin_ips) == 0:
        if "cloudflare" in detected_cdns:
            findings.append(IntelligenceFinding(
                entity="No origin IP found behind Cloudflare",
                type="CDN: Origin Not Found",
                source="CDNOriginFinder",
                confidence="Medium",
                color="slate",
                threat_level="Informational",
    ...
    if findings:
        ...
        summary_lines = [
            f"CDNs detected: {', '.join(cdn_names) if cdn_names else 'None'}",
            f"Origin IPs: {origin_count} high, {medium_origin} medium, {low_origin} low confidence",
            f"Total origin candidates: {origin_count + medium_origin + low_origin}",
        ]

        findings.append(IntelligenceFinding(
            entity=f"CDN Origin Scan: {origin_count + medium_origin + low_origin} origin candidates",
            type="CDN: Summary",
            source="CDNOriginFinder",
            confidence="Medium",
