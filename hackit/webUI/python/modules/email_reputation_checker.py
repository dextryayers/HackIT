...
async def url_safe_check(client: httpx.AsyncClient, domain: str) -> Dict:
    ...
    try:
        ...
        resp = await safe_fetch(client, "https://safebrowsing.googleapis.com/v4/threatMatches:find?key=",
            json=payload,
            headers={"User-Agent": UA},
...
async def url_vt_check(client: httpx.AsyncClient, domain: str) -> Dict:
    ...
    try:
        ...
        pass
    try:
        resp = await safe_fetch(client, "https://www.virustotal.com/api/v3/urls",
...
async def crawl(target: str, client: httpx.AsyncClient):
    ...
    if findings:
        ...
        findings.append(make_finding(
            ...
            ftype="Email: Security Posture Summary",
            source="EmailReputation",
            confidence="High",
            color=posture_color,
    ...
    async def detect_email_provider():
        ...
        if mx_info["has_mx"]:
            ...
            findings.append(make_finding(
                ...
                ftype="Email: Provider Detection",
                source="EmailReputation",
    ...
    async def check_web_headers():
        for proto in ["https", "http"]:
            try:
                resp = await safe_fetch(client, f"{proto}://{domain}", timeout=8.0, follow_redirects=True,
                    headers={"User-Agent": UA})
    ...
    async def check_subdomains():
        ...
        for sub in common:
            try:
                resp = await safe_fetch(client, f"https://{sub}.{domain}", timeout=5.0,
    ...
    async def check_mx_geo():
        if mx_info["has_mx"]:
            for mx in mx_info["mx_records"][:3]:
                ...
                for ip in ips[:2]:
                    try:
                        resp = await safe_fetch(client, f"http://ip-api.com/json/{ip}", timeout=8.0)
                        if resp.status_code == 200:
                            data = resp.json()
                            if data.get("status") == "success":
                                findings.append(make_finding(
    ...
    async def check_domain_risk_analysis():
        ...
        findings.append(make_finding(
            ...
            ftype="Email: Domain Classification",
            source="EmailReputation",
            confidence="High",
            color="slate",
            tags=["classification"]))
