...
#else
    {
        struct pollfd pfd = { .fd = s, .events = POLLOUT | POLLERR };
        ...
        if (n > 0 && (pfd.revents & POLLOUT)) {
            ...
            else if (so_err == ECONNREFUSED) result = 2; /* CLOSED */
            else if (so_err == ETIMEDOUT || so_err == EHOSTUNREACH || so_err == ENETUNREACH) result = 3; /* FILTERED */
            else result = 2; /* default closed */
        } else if (n == 0) {
            result = 3; /* FILTERED (timeout) */
        }
    }
#endif
