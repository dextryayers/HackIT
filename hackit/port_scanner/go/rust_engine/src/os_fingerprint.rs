...
pub fn detect_os_from_tcp_params(ttl: i32, window_size: i32, banner: &str) -> RustOSInfo {
    ...
    if ttl <= 64 {
        ...
        os_name = "Linux";
        accuracy = 60;
        evidence.push(format!("ttl={} => Linux/Unix baseline", ttl));
    } else if ttl <= 128 {
        os_family = "Windows";
        os_name = "Windows";
        accuracy = 60;
        evidence.push(format!("ttl={} => Windows baseline", ttl));
    } else {
        os_family = "Network Device";
        os_name = "Cisco/Generic";
        accuracy = 40;
        evidence.push(format!("ttl={} => Network device baseline", ttl));
    }
    ...
    if banner_lower.contains("ubuntu") {
        ...
        os_family = "Linux";
        accuracy = 95;
        evidence.push("banner contains 'ubuntu'".to_string());
        ...
        os_family = "Linux";
        accuracy = 95;
        evidence.push("banner contains 'debian'".to_string());
    } else if banner_lower.contains("centos") {
        os_name = "CentOS";
        os_family = "Linux";
        accuracy = 90;
        evidence.push("banner contains 'centos'".to_string());
    } else if banner_lower.contains("microsoft-iis") {
        os_name = "Windows Server";
        os_family = "Windows";
        accuracy = 85;
        evidence.push("banner contains 'microsoft-iis'".to_string());
        ...
        os_family = "Linux";
        accuracy = 99;
        evidence.push("banner contains 'openssh_8.9p1 ubuntu-3ubuntu0.1'".to_string());
    }

    // 3. TCP Window Size refinement
    if os_family == "Linux" && window_size == 5840 {
        accuracy += 5;
        evidence.push("window=5840 => common Linux window".to_string());
    } else if os_family == "Windows" && (window_size == 8192 || window_size == 65535) {
        accuracy += 5;
        evidence.push(format!("window={} => common Windows window", window_size));
    }

    if window_size > 0 {
        evidence.push(format!("observed window_size={}", window_size));
    }
