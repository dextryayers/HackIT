...
fn main() {
    ...
    let ip = resolve_ip(host).unwrap_or_else(|| host.parse().unwrap_or(std::net::IpAddr::V4(std::net::Ipv4Addr::new(0,0,0,0)))).to_string();
    let probe_ports: Vec<u16> = port_spec.split(',').filter_map(|s| s.trim().parse().ok()).collect();
    let mut probes = Vec::with_capacity(probe_ports.len());
    let mut open_ports = Vec::with_capacity(probe_ports.len());
